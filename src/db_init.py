...
def init_database(db_manager: DatabaseManager):
    ...
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='sms_users' AND xtype='U')
    CREATE TABLE users (
    ...
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='sms_groups' AND xtype='U')
    ...
    CREATE INDEX idx_group_members_group ON group_members(group_number);
...
#scada_sms.exe set-db-credentials "domain\username" "password"
#scada_sms.exe set-db-connection "SERVER\INSTANCE" "scada_db"

# Then initialize the database
#scada_sms.exe init-db
